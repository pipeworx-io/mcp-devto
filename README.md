...
`tools/list` at `https://gateway.pipeworx.io/devto/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/devto_get_articles \
  -H 'Content-Type: application/json' \
  -d '{"tag":"javascript","limit":10}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/devto_get_articles`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
