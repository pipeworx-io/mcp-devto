...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'DEV.to');
}

const BASE = 'https://dev.to/api';

/**
 * dev.to 403s a request with an EMPTY User-Agent, and the Workers runtime sends
 * none — so every call from the gateway was refused while the same request from
 * anywhere with a UA succeeded. Third pack tonight with this exact fault
 * (phishtank #574, chess #578, this one #579).
 *
 * Measured against dev.to/api/articles, varying only the header:
 *   User-Agent: ''        -> 403, empty body
 *   User-Agent: curl/8.0  -> 200
 *   descriptive UA        -> 200
 * The bar is only "non-empty". We were sending nothing.
 */
const UA = 'pipeworx-mcp-devto/1.0 (+https://pipeworx.io)';
...
async function devtoGet(path: string, params?: Record<string, string>): Promise<unknown> {
  ...
  if (!res.ok) {
    ...
    throw await httpError(res, 'DEV.to API error');
  }
  return res.json();
}
